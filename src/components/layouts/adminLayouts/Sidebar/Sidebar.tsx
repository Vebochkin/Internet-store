    ...
    <div className={`${classes.sidebar} ${isOpen && classes.open}`}>
      ...
      <nav className={classes.navigation}>
        <ul className={`${isOpen && classes['list-opened']} ${classes.list}`}>
          <SideBarItem isOpen={isOpen} title={'Лист заказов'} icon={<OrderIcon />} link={PATHS.orders} />
          <SideBarItem isOpen={isOpen} title={'Ассортимент'} icon={<ProductIcon />} link={PATHS.products} />
          <SideBarItem isOpen={isOpen} title={'Конфигурации'} icon={<CategoryIcon />} link={PATHS.settings} />
        </ul>
      </nav>
    </div>
  );
